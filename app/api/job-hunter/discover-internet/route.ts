...
export async function GET(request: Request) {
  ...
  const uniqueUrlsToProcess = getUniqueUrls(urls)
  const urlsToProcess = uniqueUrlsToProcess.slice(0, MAX_PROCESSED_SOURCES)
  ...
  return NextResponse.json({
    ...
    uniqueUrlsFound: uniqueUrlsToProcess.length,
